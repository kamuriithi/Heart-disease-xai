...
CDAM . Chuka University | Python for Data Science & Machine Learning Project
"""
...
with st.sidebar:
    st.markdown("""
    <div style='text-align:center; padding: 16px 0 8px;'>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_form:
        ...
        with st.expander("❤️ Cardiac Symptoms", expanded=True):
            chest_pain = st.selectbox("Chest Pain Type", ["asymptomatic", "nonanginal", "nontypical", "typical"])
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        st.markdown('<div class="section-header">📊 Mean |SHAP| Feature Importance</div>', unsafe_allow_html=True)
    ...
    # ── Tab 2: Beeswarm ─────────────────────────────────────────────────────
    with tab2:
        st.markdown('<div class="section-header">🐝 SHAP Beeswarm (Summary) Plot</div>', unsafe_allow_html=True)
    ...
    with tab3:
        st.markdown('<div class="section-header">🌊 Individual Patient Waterfall</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Display Chuka University Logo
    if os.path.exists("dS_lifecycle.png"):
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    Cardiovascular disease, its risk factors, symptoms, and prevention
    How the dashboard's ML models (Logistic Regression, KNN, SVM, Random Forest, XGBoost) work
    SHAP (SHapley Additive exPlanations) and explainable AI concepts
    The clinical features used: Age, Sex, ChestPain, RestBP, Cholesterol, Fasting Blood Sugar,
    RestECG, MaxHR, Exercise Angina, ST Depression (Oldpeak), ST Slope, Ca (major vessels), Thalassemia
    General heart health advice and lifestyle guidance
